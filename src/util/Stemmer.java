...
public class Stemmer {
    ...
    private char[] b;
    private int i_start, /* offset into b */
            i_end, /* offset to end of stemmed word */
            j, k;
    private static final int INC = 50;
    private static int Process_Step = 0;
    ...
    public void add(char ch) {
        if (i_start == b.length) {
            char[] new_b = new char[i_start + INC];
            for (int c = 0; c < i_start; c++) {
                new_b[c] = b[c];
            }
            b = new_b;
        }
    ...
    public void add(char[] w, int wLen) {
        if (i_start + wLen >= b.length) {
            char[] new_b = new char[i_start + wLen + INC];
            for (int c = 0; c < i_start; c++) {
                new_b[c] = b[c];
            }
            b = new_b;
    ...
    private final boolean cons(int i) {
        switch (b[i]) {
            ...
            case 'u':
                return false;
            case 'y':
                return (i_start == 0) ? true : !cons(i_start - 1);
            default:
                return true;
        }
    }
